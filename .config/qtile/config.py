...
groups = [
    ...
    Group(name="2", label="WEB", matches=[Match(role=["browser"])]),
...
layout_style = dict(border_focus=GIGAS, border_width=3, margin=7)
layouts = [
...
def run(command):
    ...
    try:
        subprocess.check_call(["pgrep", cmd_list[0]])
    except:
        ...
        top=bar.Bar(
            [
                ...
                widget.GroupBox(
                    borderwidth=2,
                    inactive=GRAY,
                    this_current_screen_border="fffff0",
                ...
                widget.WindowName(),
                widget.CheckUpdates(
                    custom_command="checkupdates", display_format="{updates}:ﮮ"
                ),
